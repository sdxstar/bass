...
func NewDefaultBass() *TBass {
	...
	bassPlayer := new(TBass)
	BASS_Init(-1, 44100, 0, 0, 0)
	return bassPlayer
}

func NewBass(device int) (*TBass, error) {
	//fmt.Println(bass.BASS_GetVersion()>>16, bass.BASSVERSION)
	if BASS_GetVersion()>>16 != BASSVERSION {
		panic("版本与头文件不符合！")
	}
	bassPlayer := new(TBass)
	b := BASS_Init(device, 44100, 0, 0, 0)
	if !b {
		return nil, ErrorFromCode()
	}
	return bassPlayer, nil
}
...
func (c *TBass) GetVolume() (uint32, error) {
	...
	return uint32(val * 100), errorFromCode()
}
...
func (c *TBass) SetDevice(val uint32) error {
	BASS_ChannelSetDevice(c.hstream, val)
	return errorFromCode()
}

// GetDevice 获取播放设备
func (c *TBass) GetDevice() (uint32, error) {
	return BASS_ChannelGetDevice(c.hstream),
		errorFromCode()
}
